...
# Define a callback function that updates the outcome text based on user input
@app.callback(Output('outcome-text', 'children'),
               [Input('new-game-button', 'n_clicks'), 
               Input('yes-button', 'n_clicks'), 
               Input('no-button', 'n_clicks')])
def check_guess(reset_clicks, yes_clicks, no_clicks):
    global correct_guesses, total_guesses, opacity_current, stock_data
    guess = None
    ...
    # Calculate the percentage change between the start and end of the selected period
    if guess != None:
    ...
    success_rate = 100 if total_guesses == 0 else round(
...
# Define a callback function that updates the candlestick chart
@app.callback(Output('candle-chart', 'figure'), 
            [Input('new-game-button', 'n_clicks'), 
            Input('yes-button', 'n_clicks'), 
            Input('no-button', 'n_clicks')])
def generate_new_chart(reset_clicks, yes_clicks, no_clicks):
    global fig, opacity_current, stock_data, start_index, end_index
    ...
    fig = go.Figure(data=[go.Candlestick(x=stock_data.index[start_index:end_index], open=stock_data['Open'][start_index:end_index], high=stock_data['High'][start_index:end_index], low=stock_data['Low'][start_index:end_index], close=stock_data['Close'][start_index:end_index])])
    fig.update_layout(title=f'Candlestick Chart for GOOG', xaxis_title='Date', yaxis_title='Price')
    ...
    # Add rectangle shape covering last 10 candles
    fig.add_shape(
        type="rect",
